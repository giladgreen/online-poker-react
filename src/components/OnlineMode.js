...
class OnlineMode extends Component {
    ...
    componentDidMount() {
        ...
        this.props.socket.on("gamesData", (games) => {
        ...
        this.props.socket.on("onerror", ({message}) => this.setState({showError:message}));
        this.props.socket.on("gameCreated", (game) => {
        ...
        this.props.socket.on("connected", (data) => {
            ...
            if (!this.state.games || this.state.games.length ===0){
                this.props.socket.emit('getGames',savedExistingGameIds);
    ...
    render() {
        ...
        if (onlineGame){
            ...
            return <OnlineGame gameId={gameId} socket={this.props.socket}/>
        }
